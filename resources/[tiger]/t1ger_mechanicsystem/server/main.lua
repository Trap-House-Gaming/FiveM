...
AddEventHandler('mechanicsystem:server:setBoss', function(data)
    ...
    if mech_shops[data.shop.id].boss ~= nil then 
        ...
        if next(data.shop.employees) ~= nil then 
            for k,v in pairs(data.shop.employees) do
                if v.identifier == mech_shops[data.shop.id].boss then
        ...
        if old_boss ~= nil then
            Core.Player.SetPlayerJob(old_boss, 'unemployed', 0)
            TriggerClientEvent('mechanicsystem:client:setMechanicId', old_boss, 0, false)
        end
    end
...
RegisterServerEvent('mechanicsystem:server:removeAccountMoney')
AddEventHandler('mechanicsystem:server:removeAccountMoney', function(id, amount)
    MinusMechanicAccount(id, amount)
end)

RegisterServerEvent('mechanicsystem:server:addAccountMoney')
AddEventHandler('mechanicsystem:server:addAccountMoney', function(id, amount)
    PlusMechanicAccount(id, amount)
end)
...
AddEventHandler('mechanicsystem:server:tryRecruit', function(data, args)
    ...
    local foundPlayer, foundShop = false, 0
...
GetMechanicShopFromId = function(id)
    if mech_shops ~= nil and next(mech_shops) then 
        if mech_shops[id] ~= nil and next(mech_shops[id]) ~= nil then 
            return mech_shops[id]
        end
    end
    return nil
end
